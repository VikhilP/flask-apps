...
class NoSpecial:
    def __init__(self, message=None):
        if not message:
            message = 'Please dont use special characters' # If no message chosen, then this default message is returned.
        self.message = message

    def __call__(self, form, field):
        if field.data.lower().isalnum() == False:
            raise ValidationError(self.message)
...
class myForm(FlaskForm):
    username = StringField('Username', validators=[
        ...
        UserCheck(message="Username is unavailable",banned = ['root','admin','sys']),
        Length(min=2,max=15)
        ])
